...
import { permissionsApi, ALL_PERMISSIONS, EMPLOYEE_ROLES, type PermissionUser } from '../lib/api'

const COLS = 'grid-cols-[90px_minmax(0,1fr)_minmax(0,1fr)_80px_110px_120px]'

export default function KengenListPage() {
  const [rows, setRows] = useState<PermissionUser[]>([])
  ...
  useEffect(() => {
    ;(async () => {
      setLoading(true)
      try {
        setRows(await permissionsApi.list())
      } catch (e: any) {
        setError(e.message)
      } finally {
        setLoading(false)
  ...
  return (
    <div>
      <h1 className="mb-4 text-lg font-bold text-ink2">権限マスタ</h1>
      <p className="mb-4 text-xs text-muted">ユーザーを選び、画面ごとのアクセス権限を設定します（全{ALL_PERMISSIONS.length}項目）。</p>
        ...
        {loading ? (
          ...
          <div className="space-y-1">
            <div className={`grid ${COLS} gap-3 border-b border-line2 pb-2 text-xs font-bold text-ink2`}>
              ...
              <span>所属部署</span>
              <span>ロール</span>
              <span className="text-center">権限数</span>
            ...
            {filtered.map((row) => (
              <div key={row.employeeId} className={`grid ${COLS} items-center gap-3 border-b border-line py-2 text-sm`}>
                ...
                <span className="text-center text-ink">{row.grantedCount}件</span>
                <div className="flex justify-center">
                  <button
                    onClick={() => navigate(`/admin/kengen/${row.employeeId}`)}
